...
export default class SEA {
    ...
    static async encrypt(data, pairsOrKey, opt) {
        ...
        const salt = opt.salt || this.random(9);    // if supplied use the salt
        const iv   = SafeBuffer.from(opt.iv) || this.random(15);
    ...
    static async decrypt(data, pairsOrKey, opt) {
        ...
        let decrypted;
        try {
            const salt  = SafeBuffer.from(obj.s, opt.encode);
            const iv    = new Uint8Array(SafeBuffer.from(opt.iv || obj.iv, opt.encode));
