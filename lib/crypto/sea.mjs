...
import { CRYPTO_SETTINGS, parseSEA, aeskey, rsapubkey, rsaprivkey, sha256hash, sha512hash, ecdhpubkey, ecdhprivkey, ecdsapubkey, ecdsaprivkey, isSEA } from "./util.mjs"
...
const SHA256    = 'SHA-256';             // hash (HMAC) algorithm    SHA-256
const SHA512    = 'SHA-512';             // hash (HMAC) algorithm    SHA-512
...
export default class SEA {
    ...
    static async pair(opt) {
        opt  = { ecdsa: true, ecdh: true, rsa: false, raw: false, ...opt };
        let keys, pub, priv;
        let pairs = {};
    ...
    static async hash512(data, salt, opt) {
        opt        = { encode: HEX, ...opt };
        if (data == undefined) throw "can't hash 'undefined'";
        data       = (typeof data == 'string') ? data : JSON.stringify(data);
        salt       = salt ?? this.random(9);
        const hash = await sha512hash(data + salt);
        const r    = SafeBuffer.from(hash, 'binary').toString(opt.encode)
        return r;
    }
...
*
    ...
    static pack(id) {
        ...
        const encoded = this.encodeBase64Url(shifted);
        return this.randomChars(3) + encoded + this.randomChars(2);
    }

    static unpack(packed) {
        if (typeof packed !== 'string') {
            throw new TypeError('packed id must be a string');
        }

        if (packed.length < 5) {
            throw new Error('Invalid packed id');
        }

        // Remove the random prefix/suffix.
        const encoded = packed.slice(3, -2);

        const shifted = this.decodeBase64Url(encoded);
