...
// import { TService } from "/evolux.dyncomponents"     // todo [OPEN]: introduce a SharedWorker for all browser plattforms
// const TService = SharedWorker;
...
const BaseEverblack = base => class extends base {
    ...
    /**
     * generate a multi key pair
     * contains:
     *  spub - spriv        (for signing/verifying)
     *  epub - epriv        (for encryption -> derive key)
     *  apub - apriv        (for asymmetric encryption)
     * @return {Promise<*>}
     */
    async pair() {
        // get key pairs
...
*/
    ...
    connectService() {
        // const srv = new TService('./everblackservice.mjs', { name: 'everblack', type: 'module' });
        // srv.port.start();

    }

    // expose the Service API as async methods for convenience
}
