...
import { ErrStoreExists, ErrNotAuthenticated, ErrNoPermission, ErrStoreExistsNot, ErrIdentityNotFound } from "./errors.mjs";
...
export default class PrivateStore extends Store {
    ...
    async invite(pubOrAlias) {
        ...
        let member = await this.buildMember(admin.ownerpair, identity, admin.sharedkey, false, null, this.salt);

        let member$ = await this._getMemberEntry(member.idhash);
        if (member$) {
            universe.logger.info('Private Store, member already invited');
            return this;
        }
    ...
    async modifyMemberEntry(pubOrAlias, workerfn) {
        ...
        if (enc.startsWith('@')) enc = enc.substr(1); // remove the '@' at the beginning
    ...
    async _restoreMeta() {
        ...
        let tnode = this.node[T];   // get meta data from DB node
