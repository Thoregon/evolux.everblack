...
class Request {
    ...
    async decryptRequest(item) {
        ...
        let ver       = await everblack().verify(enc, req.keys.pub);
        let dec       = await everblack().decrypt(ver, secret);
        this.payload  = dec;
        this.keys     = req.keys;
        this.ctrl     = req.ctrl;       // todo: decrypt
