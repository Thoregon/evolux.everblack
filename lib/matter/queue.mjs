 ...
 * todo [OPEN]: introduce a (sub) queue for every user (shared secret hash)
 *
 * @author: Bernhard Lukassen
 */

import TwinStore    from "./twinstore.mjs";
...
export default class Queue extends TwinStore {
    ...
    request(payload, timeout) {
        return new Promise(async (resolve, reject) => {
            ...
            req.reqnode = await this.contentnode.add(enc);
    ...
    async handle(fn) {
        ...
        this.contentnode.map().on(async (item, key) => {
...
class Request {
    ...
    async encryptRequest() {
        ...
        let req    = {
            ...
            // todo [OPEN]: implement control data (encrypted)
            // todo [REFACTOR]: better switch to the idhash for authentication because this can only be created by the member or the admin --> see decryptRequest()
            ctrl   : member ? await pair.sharedEncryptAndSign(this.owner, { member: { pub: member.pub, epub: member.epub } }) : '',
            payload: sig
        }
    ...
    async decryptRequest(item) {
        ...
        if (this.queue.onlyMembers) {
            ...
            // todo [REFACTOR]: better switch to the idhash for authentication because this can only be created by the member or the admin --> see encryptRequest()
