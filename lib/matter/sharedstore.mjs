...
export default class SharedStore extends SecureStore {
     ...
     * everybody who knows the address
     * @param ghost
     */
    async withGhost(ghost) {
        if (!(ghost.pub && ghost.priv && ghost.epub && ghost.epriv)) throw ErrNoIdentity();
        let tnode = this.metanode;
        tnode.g = {...ghost};   // persist for public read access
        await this.invite(ghost, false);
    }

    async joinAsGhost() {
        let tnode = this.metanode;
        let ghost = await tnode.g;
