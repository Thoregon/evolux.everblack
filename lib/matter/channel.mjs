...
export default class Channel extends PrivateStore {
    ...
    async send(message) {
        ...
        // store it
        return this.node.channel.add(enc);
    }
    ...
    // todo [OPEN]: encrypted messages needs to be signed.
    //  - verify in the firewall adapter, reject unsigned/wrong signed
    //  - do without exposing pub keys of members.
    //  - introduce an additional 'write' keypair to implement read/write permissions -> sign entry which is encrypted by the shared key
