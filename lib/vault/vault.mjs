...
export default class Vault {
    ...
    async seal(key) {
        // encode 'kv'
        await this._encode(key || this._key);
    ...
    /**
     * creates a secret key with a keypair and a public key
     *
     * @param {String}  id  - id of the generated secret
     * @param {Pair}    pairid
     * @param {String}  pubkey
     */
    async createSharedSecret(id, pairid, pubkey) {
        if (!this.kv) throw ErrVaultSealed();
    ...
    async save() {
        if (!this.store) throw ErrVaultStoreMissing();
        await this._encode(this._key);     // todo: possibly dangerous holding key in memory
